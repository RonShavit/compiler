...
.cont:
        ...
        inc rdi
        shl rdi, 3
        call malloc  ;rax<-allocated mem addr

        mov     rsi, qword [rbp + 8*2]   ; rsi = env
        lea     rdi, [rax + 8]          ; rdi = &dst[1]
        xor     rcx, rcx                ; i = 0
...
.copy_loop:
        ...
        mov     rdx, qword [rsi + rcx*8] ; load src[i]
        mov     qword [rdi + rcx*8], rdx ; store dst[i+1]
...
.after_loop:
        ...
        pop rax
            ...
            ; r8  = p
    ; rbx = dst array (p quadwords)
    ; rax = previous dst array
...
.copy_params:
        ...
        mov     rdx, [rbp + 8 * (4 + rcx)]   ; load [rbp + 8*(4+i)]
        mov     [rbx + 8 * rcx], rdx         ; rbx[i]

        inc     rcx
        jmp     .copy_params

.done:
        mov     [rax + 8 * 0], rbx            ; rax[0] = rbx
        ret
