...
module Tag_Parser : TAG_PARSER = struct
  ...
  let rec macro_expand_qq = function
   ...
   |ScmPair(
    ...
    |ScmPair(ScmSymbol("unquote-splicing"), _) as sexpr ->
      ScmPair(
        ScmSymbol("quote"), sexpr
      )

    |ScmPair(
      ScmPair(
        ScmSymbol("unquote-splicing"),
        ScmPair(sexprA,ScmNil)
      ),
      sexprB
    )->
      let sexprB = macro_expand_qq sexprB in
    ...
    |ScmPair(sexprA,sexprB)->
      let sexprA = macro_expand_qq sexprA in
      let sexprB = macro_expand_qq sexprB in
    ...
    |ScmVector sl ->
      let scheme_list = 
      List.fold_right (fun sexp acc->ScmPair(sexp,acc))
      sl
      ScmNil in
      let expended_list = macro_expand_qq scheme_list in
      ScmPair(ScmSymbol "list->vector", ScmPair(expended_list, ScmNil))
    |x -> x
  ;;
  ...
  |ScmPair(rib,ribs) ->
    match rib with 
    |ScmPair(
      ScmSymbol "else", sexprs
    ) -> 
      let sexprs_list , _ = scheme_list_to_ocaml sexprs in
      let begin_sexp = expand_begin sexprs_list in
      begin_sexp
    |ScmPair(
      test,
      ScmPair(
      ...
      let value = 
        ...
        let rest = ScmPair(
          ScmSymbol "rest",
          ScmPair(
            gen_empty_lambda (macro_expand_cond_ribs ribs),ScmNil
          )
        ) in
        let vars = (scheme_list_of_ocaml_list [value;f;rest] ScmNil) in
        let then_exp = ScmPair(
          ScmPair(ScmSymbol "f", ScmNil),
          ScmPair (ScmSymbol "value", ScmNil)
        ) in
        let else_exp = ScmPair(ScmSymbol "rest", ScmNil) in
        let body = [ScmSymbol "if"; ScmSymbol "value"; then_exp; else_exp ] in
        let body = scheme_list_of_ocaml_list body ScmNil in
        let sexp = [ScmSymbol "let"; vars ; body] in
        let sexp = scheme_list_of_ocaml_list sexp ScmNil in
        sexp
    ...
    |ScmPair(
      ...
      let sexps_list, last = scheme_list_to_ocaml sexps in
      let sexps_begin  = expand_begin sexps_list in
    ...
    |uknown -> raise (X_not_yet_implemented ("Type "^(string_of_sexpr uknown)^ " is not implemeted\n"))
  |x -> ScmString "Not yet im"
  ;;
  ...
  let rec tag_parse sexpr =
    ...
    | ScmSymbol var ->
       ...
       then raise (X_syntax ("Variable " ^ var ^ " cannot be a reserved word in "^ (string_of_sexpr sexpr)))
    ...
    let rec run expr params env =
      ...
      | ScmIf (test, dit, dif) ->
         ScmIf'(run test params env,
                ...
                run dif params env)
      | ScmSeq exprs ->
         let annotated = List.map
          (fun exp->run exp params env)
          exprs
          in
         ScmSeq'(annotated)
      | ScmOr exprs ->
      ...
      | ScmLambda (params', Simple, expr) ->
        let ext_env = List.append [params] env in
        let expr' = run expr params' ext_env in
        ScmLambda'(params', Simple, expr')
  ...
  let annotate_tail_calls = 
    let rec run in_tail = function
      ...
      | ScmApplic' (proc, args, app_kind) ->
         ...
         match args with
         |[] -> ScmApplic'(run false proc, args, if in_tail then Tail_Call else Non_Tail_Call)
         |arg::args -> ScmApplic'(run false proc, runl false arg args, if in_tail then Tail_Call else Non_Tail_Call)
    ...
    let rec run params env = function
      ...
      | ScmApplic' (proc, args, Non_Tail_Call) -> 
        let args_code = String.concat "\n" (List.rev (List.map (fun exp' -> (run params env exp') ^ "\tpush rax") args)) in
         ...
         let args_code = args_code ^ "\n\tpush qword " ^ n ^" ; Push n (num of args)\n" in
         let proc_code = run params env proc in
         let app_code = args_code ^ proc_code in
         let app_code = app_code ^ (Printf.sprintf "\tcmp byte [rax], T_closure ; check if proc is a closure\n") in
         ...
         (*let app_code = app_code ^ "\t;this is where we would push rax.env\n" in*)
         let app_code = app_code ^ "\tmov rbx, [rax + 1 + 8] ; rbx <-- rax.code\n" in
         let app_code = app_code ^ "\tcall rbx\n" in
         (*let app_code = app_code ^ "\t;this is where we would call rax.code\n" in*)
         app_code
      ...
      | ScmApplic' (proc, args, Tail_Call) -> 
        ...
        let args_code = args_code ^ "\n\tpush qword " ^ m ^" ; push m (num of args)\n" in
        let proc_code = run params env proc in
        let app_code = args_code ^ proc_code in
        let app_code = app_code ^ (Printf.sprintf "\tcmp byte [rax], T_closure ; check if proc is a closure\n") in
        let app_code = app_code ^ "\tjne L_error_non_closure\n" in
